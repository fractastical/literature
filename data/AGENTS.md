...
The `literature/` directory serves as the central repository for academic literature search, PDF storage, reference management, and research paper organization. It integrates with the `infrastructure/literature/` module to provide a complete literature management workflow.
...
├── failed_downloads.json # Failed PDF download attempts (for retry)
...
```json
{
  ...
  "entries": {
    "smith2024machine": {
      ...
      "pdf_path": "literature/pdfs/smith2024machine.pdf",
...
python3 -m infrastructure.literature.cli search "machine learning transformers" --limit 10

# Search and download PDFs
python3 -m infrastructure.literature.cli search "quantum computing" --download

# Use specific sources
python3 -m infrastructure.literature.cli search "neural networks" --sources arxiv,semanticscholar
```
...
3. Downloads PDFs to `literature/pdfs/`
4. Generates summaries to `literature/summaries/`
...
python3 -m infrastructure.literature.cli library export --output my_library.json
```
...
python3 -m infrastructure.literature.cli library refresh
```
...
cp literature/library.json literature/references.bib backup/
...
cat literature/failed_downloads.json

# Retry specific paper
python3 -m infrastructure.literature.cli download smith2024machine
...
cp literature/library.json literature/library.json.backup

# Rebuild from BibTeX (if needed)
python3 -m infrastructure.literature.cli library rebuild
```
